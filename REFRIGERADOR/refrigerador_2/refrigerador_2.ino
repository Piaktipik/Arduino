...
void loop() {
  ...
  if (menu > 0){
    ...
    if (menu > 1){
      ...
      if (menu == 2){
        if (boton == 2){boton=0;beep(5);(pos_sub_menu>0)?(pos_sub_menu = pos_sub_menu-1)%sub_menu_max:pos_sub_menu = sub_menu_max-1;}
        if (boton == 3){boton=0;beep(5);pos_sub_menu = (pos_sub_menu+1)%sub_menu_max;}
        
        if (boton == 1){boton=0;beep(5);menu = 3; valor = 0;} // activo la edicion del sub menu, inicio valor de calibracion en 0
      }
    }
  ...
  if (millis()>(tiempo_bot+300)){
    revisar_botones();
    if (boton > 0){if(menu == 0){menu = 1;boton=0;beep(5);}tiempo_menu = millis();}
    tiempo_bot = millis();
  }
