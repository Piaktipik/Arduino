...
void accion_serial(){
  ...
  int c = 0;                                       // contador de espera
    ...
    if(nomb == 'O' || nomb == 'C' || nomb == 'T' || nomb == 'L'|| nomb == 'M'){
      ...
      }// fin si el mensaje es para este globo
    
    }  // pregunto si es valido
    else{
      return;    // si no encuntro la B salgo del ciclo
    }
...
String leer_direccion(){
  ...
  while(Serial.available() < 16){                // leo la direccion de destino del mensaje
    if(c>50){c=0;return "F";}else{delay(1);c = c+1;} // si demora mas de 50ms -> reinicio el loop
  }
  for(int i=0;i<16;i++){                         // recorro el buffer (leo direccion)                  
...
void confirmacion_dato(){
  ...
  while(Serial.available() < 12){                                // leo la hora y el caracter de finalizacion
    if(c>50){c=0;return;}else{delay(1);c=c+1;}        // si demora mas de 100ms -> se cancela el comando
  }
  ...
  if(Serial.read() != 'F'){return;}                    // si no es F el paquete es invalido -> salgo de la funcion
...
void configuracion_tiempo(){                               // si es T: configuro el tiempo actual
  ...
  int c = 0;                                               // reinicio el contador de espera
  while(Serial.available() < 12){                          // leo la hora y el caracter de finalizacion
    if(c>50){c=0;return;}else{delay(1);c=c+1;}             // si demora mas de 50ms -> reinicio el loop
  }
  for(int i=0;i<12;i++){                                   // recorro el buffer                   
...
void configurar_pin(){                                            // si es C: configuro el tiempo actual
  ...
  ult = Serial.read();
  // pregunto si el ultimo caracter es F
  if(char(ult) != 'F'){return;}                         // si no es F el paquete es invalido -> salgo de la funcion
  // si el paquete es valido hago el cambio y envio confirmacion
  pinMode(c,OUTPUT);
  if (c1 == 255 || c1 == 0) digitalWrite(c,c1);
  else analogWrite(c,c1);
  // envio mensaje indicando que el puerto ha sido configurado
