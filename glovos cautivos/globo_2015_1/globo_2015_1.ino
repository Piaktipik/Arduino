...
void setup(){
  ...
  //eeprom_tiempo(5);                                           // configuro tiempo de muestreo a 1 seg - para precargar la eeprom
...
void loop(){
  ...
  // se busca la B de la base para empezar el proceso
  ...
  // si no se ha configurado la hora se pide la hora o si el tiempo requiere ser sincronizado envio el mensaje para sincronizarlo
  if(timeStatus() == timeNotSet || timeStatus() == timeNeedsSync){Serial.println("GI" + ID + "F");delay(200);goto ini;}
...
void enviar_datos(){
  ...
  Serial.print("F");
  // ----------------- salto de linea (usado por el receptor para cargar el mensaje) ----------------- 
  Serial.println();
}
