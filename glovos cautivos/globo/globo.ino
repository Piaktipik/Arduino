  ...
  String ID = "";          // serial del xbee que se usa para la identificacion del globo
  ...
  byte c = 0;              // contador usado para los retardos
  ...
  void setup(){
    // se ejecuta solo a la primera corrrida de setup ( ya que para conectar con los xbee el codigo entra varias veces a setup )  
    if (c1==0){
      Serial.begin(9600);            // inicio comunicacion serial a 9600
      c1=1;                          // variable usada para entrar solo una vez
    ...
    while(Serial.available() < 1){                       // si no he recivido almenos 2 valores -> espero (OK)
      if(c>150){c=0;setup();}else{delay(10);c=c+1;}      // reviso si envio el OK cada 10ms, si demora mas de 1500ms reintenta la comunicacion 
    }    
    ...
    while(Serial.available() < 14) {                     // reviso si llego la dirrecion (16 bits) cada 10ms
      if(c>200){c=0;setup();}else{delay(10);c=c+1;}      // si demora mas de 2000ms reintenta la comunicacion
    }
    c=0;                                                 // reinicio el contador de retardos de 10ms
  ...
  void loop(){
    ...
    if (c1==1){                                          // pregunto si es la primera ejecucion                       
      lserial();                                           // limpio el buffer
      c1=0;                                                // evito que se vuleva a ejecutar
    }
