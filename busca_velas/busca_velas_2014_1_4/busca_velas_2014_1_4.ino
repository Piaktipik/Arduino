...
byte velo_moto_izqu = 80; // izquierdo
byte velo_moto_dere = 85; // derecho

byte velo_esta = 80;      // velocidad entandar de el desplazamiento
...
byte ruid_vela = 30;                   // variable con el ruido normal de los sensores de vela ------------------------------------------------------- referencia velas --------------------------------------

byte defe_ruid_vela = 30;              // valor normal del ruido de la vela
...
unsigned int tiem_apag = 1000;         // tiempo que el robot usa para apagar la vela en ms ---------------------------------------------------------- timepo de apagado vela ---------------------------------

unsigned long ulti_vist_vela = 0;      // variable usada para reiniciar la deteccion de vela en caso de perdera
...
int rlinea = 400;               // defino el timepo de espera de los sensores -------------------------------------------------------------------------- referencia linea ------------------------------------
...
void loop() {
  ...
  if(cont_line_dere > 1 && cont_line_izqu > 1){veo_line_fren = true;}        // si hay linea en frente
  if(cont_line_dere > 1 || cont_line_izqu > 1){veo_line_late = true;}        // si veo linea de lado
  
  // ------------------------------------ apago la vela ------------------------------------  
  
  // si veo vela la apago si: esto a 20 de un obstaculo y veo una linea de frente y estoy en frente de algo caliente
  if((medi_dist_ultr_fron < 20 && veo_line_fren) && apag_vela){
    ...
    velo_moto_izqu = velo_esta;velo_moto_dere = velo_esta + valo_ajus_velo; // configuro velocidades
    fan(0);atras();delayy(400);            // apago el ventilador marcho atraz por 400 ms
  ...
  if (veo_obst && !apag_vela){goto evadiendoo;}  // si hay obstaculos y no estoy apagando una vela -> no reviso la vela y evado obstaculo
  ...
  if (veo_line_fren && !apag_vela){
    ...
    atras();delay(200);          // retrocedo ya que la linea esta de frente
    if(cont_line_dere > cont_line_izqu){izquierda();}else{derecha();}
    delayy(200);      // escapo para el lado con al cual tenga menos sensores en alto 
  }
  
  else if(veo_line_late && !apag_vela){
    velo_moto_izqu = velo_esta;velo_moto_dere = velo_esta + valo_ajus_velo;  // configuro velocidad de motores
    if(cont_line_dere > cont_line_izqu){izquierda();}else{derecha();}
    delayy(200);      // escapo para el lado con al cual tenga menos sensores en alto 
  }
  ...
  // si pierdo de vista la vela durante mas de 1000 ms la doy por perdida
  if (ulti_vist_vela + 1000 < millis()){
  ...
  if(senvela[10] > ruid_vela){
    velo_moto_izqu = velo_esta;
    velo_moto_dere = velo_esta + valo_ajus_velo;  // ajusto la velocidades al maximo
    adelante();                          // me acerco al robot
  } 
...
//  }
  ...
  if((aux_tiem_atra + 200) < millis()){
    if(medi_dist_ultr_dere == valo_pasa_1 && medi_dist_ultr_fron == valo_pasa_2 && medi_dist_ultr_izqu == valo_pasa_3){
      ...
      delayy(200);
      buz(0);
      if(dire_busq_vela){izquierda();}else{derecha();}delayy(200);  // tomo decicion de nuevo rumbo en la direccion contraria al giro que el robot hizo hacia la vela
      adelante();
      delayy(400);
    }
