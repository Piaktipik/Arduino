...
void setup(){
  ...
  SetTunings(1.2,3.75,0);//kp=2, ki=0.03, kd=2
...
void loop(){ 
  float lin;  // posicion de la linea
  medirlinea();                // mido los sensores de linea
  lin = calcular_linea();    // calculo la posicion de la linea por pesos
  Input = -lin;               // pongo como entrada del controlador el valor medido por peso
...
void seguir_1(float LINEA){
  ...
  if (abs(LINEA) < 0.2 && (ult_d || ult_i)){
    if(ult_d){
      motor(0,0.8);     // motor derecho
      motor(1,-0.3);   // motor izquierdo
    }else if(ult_i){
      motor(0,-0.3);   // motor derecho
      motor(1,0.8);     // motor izquierdo
    }  
  }
