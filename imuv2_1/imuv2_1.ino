...
#define GYR_ADDRESS (0xD6 >> 1) // datasheetserial

#define MAG_ADDRESS (0x3C >> 1) // datasheet
#define ACC_ADDRESS (0x32 >> 1) // datasheet
...
void setup(){
  ...
  //SerialSerial1.begin(115200); // begin serial comm.
...
//    
  ...
  if(Serial.available() > 0){datoserial(Serial.read());}
  // -------------- fin  del programa -------------- 
...
void datoserial(char dato){
    ...
    if(dato == 'v'){
      delay(100);
      vel1 = Serial.read(); // velocidad de motores
      vel2 = Serial.read();
      error = 0; // indico que entro en modo toma de datos
    }
}
